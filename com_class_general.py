...
def classifiers_sets(datapath):
    ...
    #datasets = [make_moons(noise=0.3, random_state=0),
     ...
     #           linearly_separable
     #           ]
    mat = loaddataset(r"./data/casia_all.mat")
    data = mat['data']
    modeldata = loadmodel("./result/pca_casia/8model.m",data[:10000])
    labeldata = {"data":modeldata,"labels":mat['labels'][:10000]}
    #print labeldata
    datasets = [labeldata]
    ...
    for ds in datasets:
        ...
        X = labeldata['data']
        print X
        thresh_min = np.amin(X)
        thresh_max = np.amax(X)
        X -= thresh_min;
        y = labeldata['labels']
        print y
...
def loaddataset(datapath):
    if datapath.endswith('.pkl'):
        ...
        d = sio.loadmat(datapath)
    return d
def loadmodel(modelpath,data):
    model = joblib.load(modelpath)
    return model.transform(data)
if __name__=="__main__":
    classifiers_sets("./result/pca_casia/result.pkl")
